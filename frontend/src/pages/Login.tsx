...
export default function Login() {
  ...
  // We use a CSS animation state to pause/play the background
  const [isAnimationPlaying, setIsAnimationPlaying] = useState(true);

  const handleModeChange = (newMode: 'signin' | 'signup') => {
    setMode(newMode);
    setIsAnimationPlaying(false); // Stop animation when button is clicked
  };
          ...
          <div className="w-full bg-white/10 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-white/20 animate-fade-in">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                <input type="password" required className="w-full bg-white/20 border border-white/30 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-white/50" placeholder="Create a password" />
              </div>
              <div>
                <label className="block text-gray-200 text-sm font-medium mb-1">Re-enter Password</label>
                <input type="password" required className="w-full bg-white/20 border border-white/30 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-white/50" placeholder="Confirm your password" />
              </div>
              <button type="submit" className="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold text-lg hover:bg-indigo-700 transition-colors mt-6 shadow-lg cursor-pointer">
                Register
              </button>
